...
if ! gh repo view --json nameWithOwner &> /dev/null; then
    echo -e "${YELLOW}⚠️  Repository not recognized by GitHub CLI${NC}"
    ...
    if ! gh repo view --json nameWithOwner &> /dev/null; then
        echo -e "${RED}❌ Failed to recognize repository${NC}"
        exit 1
    fi
    echo -e "${GREEN}✓ Repository recognized${NC}"
fi

# 기존 릴리스 확인
echo -e "${BLUE}🔍 Checking if release v${VERSION} already exists...${NC}"
